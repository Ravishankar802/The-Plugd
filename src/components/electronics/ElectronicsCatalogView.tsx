...
import ElectronicsCarouselSection from "./ElectronicsCarouselSection";
import ElectronicsProductCard from "./ElectronicsProductCard";
import {
  ELECTRONICS_SECTIONS,
  getFullElectronicsCatalog,
  type ElectronicsProduct,
} from "@/lib/electronics-catalog";

interface ElectronicsCatalogViewProps {
  isLoggedIn: boolean;
  initialQuery?: string;
  catalogDbMap?: Record<string, string>;
}

const SUBCATEGORY_NAV_ITEMS = [
  { id: "all", label: "All", icon: Sparkles },
  { id: "trending", label: "Trending", icon: Sparkles, sectionId: "trending" },
  { id: "laptops", label: "Computers", icon: Laptop, sectionId: "laptops-computers" },
  { id: "audio", label: "Audio", icon: Headphones, sectionId: "audio-store" },
  { id: "gaming", label: "Gaming", icon: Gamepad2, sectionId: "gaming" },
  { id: "monitors", label: "Monitors", icon: Tv, sectionId: "monitors-displays" },
  { id: "nvidia", label: "PC Hardware", icon: Cpu, sectionId: "nvidia-hardware" },
  { id: "cameras", label: "Cameras", icon: Camera, sectionId: "cameras-photography" },
  { id: "creator", label: "Creator Gear", icon: Mic, sectionId: "creator-streaming" },
  { id: "storage", label: "Storage", icon: HardDrive, sectionId: "storage-backup" },
  { id: "desk", label: "Desk Setup", icon: Keyboard, sectionId: "desk-setup" },
  { id: "networking", label: "Smart Home", icon: Wifi, sectionId: "networking-smarthome" },
  { id: "power", label: "Accessories", icon: Zap, sectionId: "accessories-power" },
  { id: "wearables", label: "Wearables", icon: Watch, sectionId: "wearables-future-tech" },
  { id: "lifestyle", label: "Lifestyle", icon: Home, sectionId: "smart-lifestyle" },
];

export default function ElectronicsCatalogView({
  isLoggedIn,
  initialQuery = "",
  catalogDbMap = {},
}: ElectronicsCatalogViewProps) {
  const [searchQuery, setSearchQuery] = useState(initialQuery);
  const [activeSectionModal, setActiveSectionModal] = useState<string | null>(null);
  const [modalSearch, setModalSearch] = useState("");

  const allProducts = useMemo(() => getFullElectronicsCatalog(), []);

  // Map products by section
  const sectionProductsMap = useMemo(() => {
    const map = new Map<string, ElectronicsProduct[]>();
    const productLookup = new Map<string, ElectronicsProduct>(allProducts.map((p) => [p.id, p]));

    for (const section of ELECTRONICS_SECTIONS) {
      const prods: ElectronicsProduct[] = [];
  ...
  const activeModalSection = useMemo(() => {
    ...
    return ELECTRONICS_SECTIONS.find((s) => s.id === activeSectionModal) || null;
  }, [activeSectionModal]);
    ...
    <div className="space-y-8">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div className="space-y-3 max-w-2xl">
            ...
            <h1 className="text-3xl sm:text-4xl md:text-5xl font-black tracking-tight text-white">
              ELECTRONICS<span className="text-orange-500">.</span>
            </h1>

            <p className="text-sm md:text-base text-zinc-300 font-normal leading-relaxed max-w-xl">
              Next-gen laptops, creator cameras, Blackwell GPUs, audiophile headphones, and dream desk setups. Add what you want to your public wishlist for your community to back.
            </p>
          ...
          <div className="w-full md:w-80 lg:w-96 space-y-2 shrink-0">
            ...
            <div className="relative flex items-center">
              ...
              <input
                ...
                placeholder="Search MacBook, Sony, RTX, Mic..."
      ...
      <section className="sticky top-[110px] md:top-[118px] z-20 -mx-4 px-4 md:-mx-6 md:px-6 py-2 bg-white/95 backdrop-blur-md border-y border-zinc-200/80">
        <div className="flex items-center gap-2 overflow-x-auto no-scrollbar py-0.5">
          {SUBCATEGORY_NAV_ITEMS.map((item) => {
      ...
      {searchQuery.trim() ? (
            ...
            <div className="rounded-[28px] border border-dashed border-zinc-300 bg-white p-8 text-center sm:text-left flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                <h3 className="text-base font-bold text-zinc-950">
                  No electronics found matching &ldquo;{searchQuery}&rdquo;
                </h3>
                <p className="mt-1 text-xs text-zinc-600 max-w-md">
                  Try searching for brand names like Apple, Sony, NVIDIA, or create a custom item.
                </p>
              </div>
              <Link
        ...
        /* 4. Full 14-Section Horizontal Carousels Catalog */
              ...
              {activeModalProducts.length > 0 ? (
                ...
                <div className="rounded-2xl border border-dashed border-zinc-300 bg-white p-8 text-center">
                  <p className="text-xs font-bold text-zinc-700">
                    No products match &ldquo;{modalSearch}&rdquo; in this section.
                  </p>
                </div>
              )}
            </div>
