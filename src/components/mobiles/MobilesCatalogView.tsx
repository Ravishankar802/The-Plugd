...
import MobilesCarouselSection from "./MobilesCarouselSection";
import MobilesProductCard from "./MobilesProductCard";
import {
  MOBILE_SECTIONS,
  getFullMobilesCatalog,
  type MobileProduct,
} from "@/lib/mobiles-catalog";

interface MobilesCatalogViewProps {
  isLoggedIn: boolean;
  initialQuery?: string;
  catalogDbMap?: Record<string, string>;
}

const MOBILE_SUBCATEGORY_NAV_ITEMS = [
  { id: "all", label: "All Mobiles", icon: Sparkles },
  { id: "trending", label: "Trending", icon: Flame, sectionId: "trending-phones" },
  { id: "flagships", label: "Flagships", icon: Crown, sectionId: "flagship-phones" },
  { id: "mid-range", label: "Mid-Range", icon: Zap, sectionId: "best-value" },
  { id: "budget", label: "Budget Phones", icon: Smartphone, sectionId: "budget-picks" },
  { id: "gaming", label: "Gaming Phones", icon: Gamepad2, sectionId: "gaming-phones" },
  { id: "camera", label: "Camera Phones", icon: Camera, sectionId: "camera-phones" },
  { id: "foldables", label: "Foldables", icon: BookOpen, sectionId: "foldables-flips" },
  { id: "flip", label: "Flip Phones", icon: Layers, sectionId: "foldables-flips" },
  { id: "5g", label: "5G Phones", icon: Zap, sectionId: "trending-phones" },
  { id: "battery", label: "Battery Phones", icon: BatteryCharging, sectionId: "best-value" },
];

export default function MobilesCatalogView({
  isLoggedIn,
  initialQuery = "",
  catalogDbMap = {},
}: MobilesCatalogViewProps) {
  const [searchQuery, setSearchQuery] = useState(initialQuery);
  const [activeSectionModal, setActiveSectionModal] = useState<string | null>(null);
  const [modalSearch, setModalSearch] = useState("");

  const allProducts = useMemo(() => getFullMobilesCatalog(), []);

  // Map products by section
  const sectionProductsMap = useMemo(() => {
    const map = new Map<string, MobileProduct[]>();
    const productLookup = new Map<string, MobileProduct>(allProducts.map((p) => [p.id, p]));

    for (const section of MOBILE_SECTIONS) {
      const prods: MobileProduct[] = [];
  ...
  const activeModalSection = useMemo(() => {
    ...
    return MOBILE_SECTIONS.find((s) => s.id === activeSectionModal) || null;
  }, [activeSectionModal]);
    ...
    <div className="space-y-8">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div className="space-y-3 max-w-2xl">
            ...
            <h1 className="text-3xl sm:text-4xl md:text-5xl font-black tracking-tight text-white">
              MOBILES<span className="text-orange-500">.</span>
            </h1>

            <p className="text-sm md:text-base text-zinc-300 font-normal leading-relaxed max-w-xl">
              From flagship iPhones and Zeiss-powered camera phones to gaming beasts, foldables, and value 5G smartphones across India. Add any phone to your public wishlist.
            </p>
          ...
          <div className="w-full md:w-80 lg:w-96 space-y-2 shrink-0">
            ...
            <div className="relative flex items-center">
              ...
              <input
                ...
                placeholder="Search iPhone, Galaxy, OnePlus, Pixel, ROG..."
      ...
      <section className="sticky top-[110px] md:top-[118px] z-20 -mx-4 px-4 md:-mx-6 md:px-6 py-2 bg-white/95 backdrop-blur-md border-y border-zinc-200/80">
        <div className="flex items-center gap-2 overflow-x-auto no-scrollbar py-0.5">
          {MOBILE_SUBCATEGORY_NAV_ITEMS.map((item) => {
            ...
            <div className="rounded-[28px] border border-dashed border-zinc-300 bg-white p-8 text-center sm:text-left flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                <h3 className="text-base font-bold text-zinc-950">
                  No mobile phones found matching &ldquo;{searchQuery}&rdquo;
                </h3>
                <p className="mt-1 text-xs text-zinc-600 max-w-md">
                  Try searching for brand names like Apple, Samsung, OnePlus, Vivo, Pixel, or create a custom item.
                </p>
              </div>
              <Link
              ...
              {activeModalProducts.length > 0 ? (
                ...
                <div className="rounded-2xl border border-dashed border-zinc-300 bg-white p-8 text-center">
                  <p className="text-xs font-bold text-zinc-700">
                    No phones match &ldquo;{modalSearch}&rdquo; in this section.
                  </p>
                </div>
              )}
            </div>
