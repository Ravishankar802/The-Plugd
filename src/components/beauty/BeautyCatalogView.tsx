...
import {
  BEAUTY_SECTIONS,
  getFullBeautyCatalog,
  type BeautyProduct,
} from "@/lib/beauty-catalog";

interface BeautyCatalogViewProps {
  isLoggedIn: boolean;
  initialQuery?: string;
  catalogDbMap?: Record<string, string>;
}

const BEAUTY_SUBCATEGORY_NAV_ITEMS = [
  { id: "all", label: "All Beauty", icon: Sparkles },
  { id: "trending", label: "Trending", icon: Flame, sectionId: "trending-beauty" },
  { id: "wishlist", label: "Wishlist", icon: Heart, sectionId: "viral-beauty-wishlist" },
  { id: "kbeauty", label: "K-Beauty", icon: Sparkles, sectionId: "k-beauty-cult-favorites" },
  { id: "makeup", label: "Makeup", icon: Wand2, sectionId: "makeup-face-base" },
  { id: "skincare", label: "Skincare", icon: Droplets, sectionId: "skincare-actives-serums" },
  { id: "lips", label: "Lip Care", icon: Heart, sectionId: "lip-favorites-oils-tints" },
  { id: "suncare", label: "Sun Care", icon: Sun, sectionId: "sun-care-spf-shields" },
  { id: "haircare", label: "Haircare", icon: Wind, sectionId: "haircare-scalp-revival" },
  { id: "hairtools", label: "Hair Tools", icon: Wind, sectionId: "hair-styling-tools" },
  { id: "fragrance", label: "Fragrance", icon: Flower2, sectionId: "perfumes-fine-fragrances" },
  { id: "bath", label: "Bath & Body", icon: Bath, sectionId: "body-mists-bath-indulgence" },
  { id: "indian", label: "Indian Beauty", icon: Crown, sectionId: "indian-heritage-ayurvedic" },
  { id: "devices", label: "Beauty Devices", icon: Wand2, sectionId: "beauty-devices-vanity-tools" },
  { id: "grooming", label: "Men's Grooming", icon: User, sectionId: "mens-grooming-beard" },
  { id: "luxury", label: "Luxury Beauty", icon: Gem, sectionId: "luxury-beauty-gift-sets" },
  { id: "gifts", label: "Gift Sets", icon: Gift, sectionId: "luxury-beauty-gift-sets" },
];

export default function BeautyCatalogView({
  isLoggedIn,
  initialQuery = "",
  catalogDbMap = {},
}: BeautyCatalogViewProps) {
  const [searchQuery, setSearchQuery] = useState(initialQuery);
  const [activeSectionModal, setActiveSectionModal] = useState<string | null>(null);
  const [modalSearch, setModalSearch] = useState("");

  const allProducts = useMemo(() => getFullBeautyCatalog(), []);

  // Map products by section
  const sectionProductsMap = useMemo(() => {
    const map = new Map<string, BeautyProduct[]>();
    const productLookup = new Map<string, BeautyProduct>(allProducts.map((p) => [p.id, p]));

    for (const section of BEAUTY_SECTIONS) {
      const prods: BeautyProduct[] = [];
  ...
  const activeModalSection = useMemo(() => {
    ...
    return BEAUTY_SECTIONS.find((s) => s.id === activeSectionModal) || null;
  }, [activeSectionModal]);
    ...
    <div className="space-y-8">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div className="space-y-3 max-w-2xl">
            <div className="inline-flex items-center gap-2 rounded-full bg-orange-500/15 border border-orange-500/30 px-3.5 py-1 text-xs font-black uppercase tracking-widest text-orange-400">
              ...
              Creator Catalog • Skincare, Makeup & Fragrance
            </div>

            <h1 className="text-3xl sm:text-4xl md:text-5xl font-black tracking-tight text-white">
              BEAUTY<span className="text-orange-500">.</span>
            </h1>

            <p className="text-sm md:text-base text-zinc-300 font-normal leading-relaxed max-w-xl">
              Viral K-Beauty glass skin essentials, dewy liquid blushes, Dyson multi-stylers, luxury French perfumes, body mists, and Ayurvedic heritage elixirs. Add beauty favorites to your public wishlist.
            </p>
          ...
          <div className="w-full md:w-80 lg:w-96 space-y-2 shrink-0">
            ...
            <div className="relative flex items-center">
              ...
              <input
                ...
                placeholder="Search Rare Beauty, Dyson, Laneige, COSRX..."
      ...
      <section className="sticky top-[110px] md:top-[118px] z-20 -mx-4 px-4 md:-mx-6 md:px-6 py-2 bg-white/95 backdrop-blur-md border-y border-zinc-200/80">
        <div className="flex items-center gap-2 overflow-x-auto no-scrollbar py-0.5">
          {BEAUTY_SUBCATEGORY_NAV_ITEMS.map((item) => {
      ...
      {searchQuery.trim() ? (
        <section className="space-y-6 animate-in fade-in duration-200">
          ...
          {filteredProducts.length > 0 ? (
            <div className="grid grid-cols-2 gap-x-2.5 sm:gap-x-3.5 gap-y-6 sm:gap-y-8 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-7 2xl:grid-cols-8">
            ...
            <div className="rounded-[28px] border border-dashed border-zinc-300 bg-white p-8 text-center sm:text-left flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                <h3 className="text-base font-bold text-zinc-950">
                  No beauty products found matching &ldquo;{searchQuery}&rdquo;
                </h3>
                <p className="mt-1 text-xs text-zinc-600 max-w-md">
                  Try searching for brand names like Rare Beauty, Laneige, COSRX, Dyson, Dior, Minimalist, YSL, or create a custom item.
                </p>
              </div>
              <Link
              ...
              {activeModalProducts.length > 0 ? (
                ...
                <div className="rounded-2xl border border-dashed border-zinc-300 bg-white p-8 text-center">
                  <p className="text-xs font-bold text-zinc-700">
                    No beauty items match &ldquo;{modalSearch}&rdquo; in this section.
                  </p>
                </div>
              )}
            </div>
