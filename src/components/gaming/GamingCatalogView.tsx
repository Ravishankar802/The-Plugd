...
import GamingCarouselSection from "./GamingCarouselSection";
import GamingProductCard from "./GamingProductCard";
import {
  GAMING_SECTIONS,
  getFullGamingCatalog,
  type GamingProduct,
} from "@/lib/gaming-catalog";

interface GamingCatalogViewProps {
  isLoggedIn: boolean;
  initialQuery?: string;
  catalogDbMap?: Record<string, string>;
}

const GAMING_SUBCATEGORY_NAV_ITEMS = [
  { id: "all", label: "All Gaming", icon: Sparkles },
  { id: "trending", label: "Trending", icon: Flame, sectionId: "trending-gaming" },
  { id: "gpus", label: "GPUs", icon: Cpu, sectionId: "gpu-powerhouse" },
  { id: "laptops", label: "Gaming Laptops", icon: Laptop, sectionId: "gaming-laptops" },
  { id: "pcs", label: "Gaming PCs", icon: Tv, sectionId: "pc-build-hardware" },
  { id: "cpus", label: "CPUs", icon: Cpu, sectionId: "pc-build-hardware" },
  { id: "monitors", label: "Monitors", icon: Tv, sectionId: "gaming-monitors" },
  { id: "keyboards", label: "Keyboards", icon: Keyboard, sectionId: "keyboards-hall-effect" },
  { id: "mice", label: "Mice", icon: Mouse, sectionId: "gaming-mice" },
  { id: "headsets", label: "Headsets", icon: Headphones, sectionId: "gaming-audio" },
  { id: "controllers", label: "Controllers", icon: Gamepad2, sectionId: "console-gaming" },
  { id: "consoles", label: "Consoles", icon: Gamepad2, sectionId: "console-gaming" },
  { id: "handhelds", label: "Handhelds", icon: Crown, sectionId: "handheld-gaming" },
  { id: "streaming", label: "Streaming", icon: Mic, sectionId: "streaming-gear" },
  { id: "sim-racing", label: "Sim Racing", icon: Gauge, sectionId: "sim-racing" },
  { id: "setup", label: "Gaming Setup", icon: Armchair, sectionId: "gaming-furniture-setup" },
  { id: "vr", label: "VR", icon: Glasses, sectionId: "vr-immersive" },
];

export default function GamingCatalogView({
  isLoggedIn,
  initialQuery = "",
  catalogDbMap = {},
}: GamingCatalogViewProps) {
  const [searchQuery, setSearchQuery] = useState(initialQuery);
  const [activeSectionModal, setActiveSectionModal] = useState<string | null>(null);
  const [modalSearch, setModalSearch] = useState("");

  const allProducts = useMemo(() => getFullGamingCatalog(), []);

  // Map products by section
  const sectionProductsMap = useMemo(() => {
    const map = new Map<string, GamingProduct[]>();
    const productLookup = new Map<string, GamingProduct>(allProducts.map((p) => [p.id, p]));

    for (const section of GAMING_SECTIONS) {
      const prods: GamingProduct[] = [];
  ...
  const activeModalSection = useMemo(() => {
    ...
    return GAMING_SECTIONS.find((s) => s.id === activeSectionModal) || null;
  }, [activeSectionModal]);
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div className="space-y-3 max-w-2xl">
            <div className="inline-flex items-center gap-2 rounded-full bg-orange-500/15 border border-orange-500/30 px-3.5 py-1 text-xs font-black uppercase tracking-widest text-orange-400">
              ...
              Creator Catalog • Gaming Battlestations
            </div>
            ...
            <p className="text-sm md:text-base text-zinc-300 font-normal leading-relaxed max-w-xl">
              Next-gen RTX 50-series GPUs, 3D V-Cache CPUs, OLED gaming displays, rapid-trigger magnetic keyboards, direct-drive sim rigs, and pro streaming setups. Add what you want to your public wishlist.
            </p>
            ...
            <div className="rounded-[28px] border border-dashed border-zinc-300 bg-white p-8 text-center sm:text-left flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                <h3 className="text-base font-bold text-zinc-950">
                  No gaming gear found matching &ldquo;{searchQuery}&rdquo;
                </h3>
                <p className="mt-1 text-xs text-zinc-600 max-w-md">
                  Try searching for brand names like NVIDIA, ROG, Razer, Logitech, PlayStation, or create a custom item.
                </p>
              </div>
              <Link
              ...
              {activeModalProducts.length > 0 ? (
                ...
                <div className="rounded-2xl border border-dashed border-zinc-300 bg-white p-8 text-center">
                  <p className="text-xs font-bold text-zinc-700">
                    No gaming gear matches &ldquo;{modalSearch}&rdquo; in this section.
                  </p>
                </div>
              )}
            </div>
