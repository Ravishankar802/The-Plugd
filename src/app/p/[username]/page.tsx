...
export default async function PublicProfilePage({ params }: PageProps) {
  // 1. Authenticate session and verify if user is the admin
  const session = await getSession();
  const adminEmail = process.env.NEXT_PUBLIC_ADMIN_EMAIL || "";
  const isAdmin = session?.email && session.email.toLowerCase() === adminEmail.toLowerCase();

  // 2. If not admin, return 404 Not Found immediately
  if (!isAdmin) {
    notFound();
  }

  // 3. Otherwise, fetch and show the promoter profile stats for the admin
